...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<-EOSQL
    ...
    GRANT USAGE ON SCHEMA $DB_NAMESPACE TO solomon_user_role;
    
    -- Create users and assign to roles
    CREATE USER zion_user INHERIT LOGIN NOREPLICATION PASSWORD '$ZION_DB_USER_PASSWORD';
    CREATE USER zion_admin INHERIT LOGIN NOREPLICATION BYPASSRLS PASSWORD '$ZION_DB_ADMIN_PASSWORD';
    CREATE USER brawler_user INHERIT LOGIN NOREPLICATION PASSWORD '$BRAWLER_DB_USER_PASSWORD';
    CREATE USER brawler_admin INHERIT LOGIN NOREPLICATION BYPASSRLS PASSWORD '$BRAWLER_DB_ADMIN_PASSWORD';
    
    -- Assign users to roles
    GRANT solomon_user_role TO zion_user, brawler_user;
    GRANT solomon_admin_role TO zion_admin, brawler_admin;
    
    -- Set search paths for existing users and roles
    ALTER ROLE solomon_user_role SET search_path TO $DB_NAMESPACE;
    ALTER ROLE solomon_admin_role SET search_path TO $DB_NAMESPACE;
