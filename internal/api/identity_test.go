...
func (ts *IdentityTestSuite) SetupTest() {
	...
	ts.project_id, ts.organization_id, _ = InitializeTestDatabase(ts.T(), ts.API, ts.Config)

	// Create user
	u, err := models.NewUser("", "one@example.com", "password", ts.Config.JWT.Aud, nil, ts.organization_id, ts.project_id)
	...
	u, err = models.NewUser("123456789", "two@example.com", "password", ts.Config.JWT.Aud, nil, ts.organization_id, ts.project_id)
	require.NoError(ts.T(), err, "Error creating test user model")
	require.NoError(ts.T(), ts.API.db.Create(u, "organization_role"), "Error saving new test user")
	require.NoError(ts.T(), u.Confirm(ts.API.db))
...
func (ts *IdentityTestSuite) TestUnlinkIdentityError() {
	...
	userWithOneIdentity, err := models.FindUserByEmailAndAudience(ts.API.db, "one@example.com", ts.Config.JWT.Aud, ts.organization_id, uuid.Nil)
	require.NoError(ts.T(), err)

	userWithTwoIdentities, err := models.FindUserByEmailAndAudience(ts.API.db, "two@example.com", ts.Config.JWT.Aud, ts.organization_id, uuid.Nil)
	require.NoError(ts.T(), err)
	cases := []struct {
		desc          string
		user          *models.User
...
func (ts *IdentityTestSuite) TestUnlinkIdentity() {
	...
	for _, c := range cases {
		ts.Run(c.desc, func() {
			...
			identity, err := models.FindIdentityByIdAndProvider(ts.API.db, u.ID.String(), c.provider, ts.organization_id, uuid.Nil)
			require.NoError(ts.T(), err)
