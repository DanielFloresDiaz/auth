...
func (p *PasswordGrantParams) Validate(a *API) error {
	...
	if p.Password == "" {
		return apierrors.NewBadRequestError(apierrors.ErrorCodeValidationFailed, "Missing password")
	}

	if p.ProjectID == uuid.Nil && p.OrganizationID == uuid.Nil {
		return apierrors.NewBadRequestError(apierrors.ErrorCodeValidationFailed, "Organization ID is required for login or at least Project ID is required for Admin Login")
	}
	return nil
}
