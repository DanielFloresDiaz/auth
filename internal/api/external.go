...
func (a *API) GetExternalProviderRedirectURL(w http.ResponseWriter, r *http.Request, linkingTargetUser *models.User) (string, error) {
	...
	if isPKCEFlow(flowType) {
		flowState, err := generateFlowState(db, providerType, models.OAuth, codeChallengeMethod, codeChallenge, nil, organization_id, project_id)
		if err != nil {
			return "", err
		}
...
func (a *API) internalExternalProviderCallback(w http.ResponseWriter, r *http.Request) error {
	...
	err = db.Transaction(func(tx *storage.Connection) error {
		...
		if targetUser != nil {
			...
			createdUser = true
			if _, user, terr = a.createAccountFromExternalIdentity(tx, r, userData, providerType, emailOptional, organization_id, project_id); terr != nil {
				return terr
			}
		}
		if flowState != nil {
...
func (a *API) createAccountFromExternalIdentity(tx *storage.Connection, r *http.Request, userData *provider.UserProvidedData, providerType string, emailOptional bool, organization_id uuid.UUID, project_id uuid.UUID) (models.AccountLinkingDecision, *models.User, error) {
	ctx := r.Context()
		...
		if decision.CandidateEmail.Verified || config.Mailer.Autoconfirm {
			...
			if !config.Mailer.AllowUnverifiedEmailSignIns {
				if emailConfirmationSent {
					return 0, nil, storage.NewCommitWithError(apierrors.NewUnprocessableEntityError(apierrors.ErrorCodeProviderEmailNeedsVerification, "Unverified email with %v. A confirmation email has been sent to your %v email", providerType, providerType))
				}

				return 0, nil, storage.NewCommitWithError(apierrors.NewUnprocessableEntityError(apierrors.ErrorCodeProviderEmailNeedsVerification, "Unverified email with %v. Verify the email with %v in order to sign in", providerType, providerType))
			}
		}
	} else {
