...
func (a *API) validateUserUpdateParams(ctx context.Context, p *UserUpdateParams) error {
	...
	if p.OrganizationID == uuid.Nil && p.ProjectID == uuid.Nil {
		return apierrors.NewBadRequestError(apierrors.ErrorCodeValidationFailed, "Organization or project ID is required")
	}
...
func (a *API) UserUpdate(w http.ResponseWriter, r *http.Request) error {
	...
	if params.Email != "" && user.GetEmail() != params.Email {
		if duplicateUser, err := models.IsDuplicatedEmail(db, params.Email, aud, user, config.Experimental.ProvidersWithOwnLinkingDomain, params.OrganizationID, uuid.Nil); err != nil {
