...
func (p *IdTokenGrantParams) Validate(a *API) error {
	...
	if p.ProjectID == uuid.Nil && p.OrganizationID == uuid.Nil {
		return apierrors.NewOAuthError("invalid request", "Organization ID is required or Project ID required for Admin login")
	}
...
func (a *API) IdTokenGrant(ctx context.Context, w http.ResponseWriter, r *http.Request) error {
	...
	if err := db.Transaction(func(tx *storage.Connection) error {
		...
		if params.LinkIdentity {
			...
			decision, user, terr = a.createAccountFromExternalIdentity(tx, r, userData, providerType, emailOptional, params.OrganizationID, params.ProjectID)
		}
