...
func (ts *MFATestSuite) SetupTest() {
	...
	// Create user
	u, err := models.NewUser("123456789", ts.TestEmail, ts.TestPassword, ts.Config.JWT.Aud, nil, ts.OrganizationID, ts.ProjectID)
	...
	u, err = models.FindUserByEmailAndAudience(ts.API.db, ts.TestEmail, ts.Config.JWT.Aud, ts.OrganizationID, uuid.Nil)
