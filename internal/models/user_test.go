...
func TestUser(t *testing.T) {
	...
	superuserURL := globalConfig.DB.URL
	if u, err := url.Parse(globalConfig.DB.URL); err == nil {
...
func (ts *UserTestSuite) TestUpdateAppMetadata() {
	...
	u, err := NewUser("", "", "", "", nil, ts.OrganizationID, ts.ProjectID)
...
func (ts *UserTestSuite) TestFindUserByEmailAndAudience() {
	...
	n, err := FindUserByEmailAndAudience(ts.db, u.GetEmail(), "test", ts.OrganizationID, ts.ProjectID)
	require.NoError(ts.T(), err)
	require.Equal(ts.T(), u.ID, n.ID)

	_, err = FindUserByEmailAndAudience(ts.db, u.GetEmail(), "invalid", ts.OrganizationID, ts.ProjectID)
...
func (ts *UserTestSuite) TestFindUsersInAudience() {
	...
	n, err := FindUsersInAudience(ts.db, u.Aud, nil, nil, "", ts.OrganizationID, uuid.Nil)
	require.NoError(ts.T(), err)
	...
	n, err = FindUsersInAudience(ts.db, u.Aud, &p, nil, "", ts.OrganizationID, uuid.Nil)
	require.NoError(ts.T(), err)
	require.Len(ts.T(), n, 1)
	...
	n, err = FindUsersInAudience(ts.db, u.Aud, nil, sp, "", ts.OrganizationID, uuid.Nil)
	require.NoError(ts.T(), err)
	require.Len(ts.T(), n, 1)
}
...
func (ts *UserTestSuite) TestIsDuplicatedEmail() {
	...
	e, err := IsDuplicatedEmail(ts.db, "david.calavera@netlify.com", "test", nil, nil, ts.OrganizationID, ts.ProjectID)
	require.NoError(ts.T(), err)
	require.NotNil(ts.T(), e, "expected email to be duplicated")

	e, err = IsDuplicatedEmail(ts.db, "davidcalavera@netlify.com", "test", nil, nil, ts.OrganizationID, ts.ProjectID)
	require.NoError(ts.T(), err)
	require.Nil(ts.T(), e, "expected email to not be duplicated", nil, nil)

	e, err = IsDuplicatedEmail(ts.db, "david@netlify.com", "test", nil, nil, ts.OrganizationID, ts.ProjectID)
	require.NoError(ts.T(), err)
	require.Nil(ts.T(), e, "expected same email to not be duplicated", nil, nil)

	e, err = IsDuplicatedEmail(ts.db, "david.calavera@netlify.com", "other-aud", nil, nil, ts.OrganizationID, ts.ProjectID)
	require.NoError(ts.T(), err)
...
func (ts *UserTestSuite) TestRemoveUnconfirmedIdentities() {
	user, err := NewUser("+29382983298", "someone@example.com", "abcdefgh", "authenticated", nil, ts.OrganizationID, ts.ProjectID)
	require.NoError(ts.T(), err)
...
func (ts *UserTestSuite) TestConfirmEmailChange() {
	user, err := NewUser("", "test@example.com", "", "authenticated", nil, ts.OrganizationID, ts.ProjectID)
	require.NoError(ts.T(), err)
	require.NoError(ts.T(), ts.db.Create(user, "organization_role"))
	...
	identity, err = FindIdentityByIdAndProvider(ts.db, user.ID.String(), "email", user.OrganizationID.UUID, user.ProjectID)
	require.NoError(ts.T(), err)
...
func (ts *UserTestSuite) TestConfirmPhoneChange() {
	user, err := NewUser("123456789", "", "", "authenticated", nil, ts.OrganizationID, ts.ProjectID)
	require.NoError(ts.T(), err)
	require.NoError(ts.T(), ts.db.Create(user, "organization_role"))
	...
	require.NoError(ts.T(), ts.db.Eager().Load(user))
	identity, err = FindIdentityByIdAndProvider(ts.db, user.ID.String(), "phone", user.OrganizationID.UUID, user.ProjectID)
	require.NoError(ts.T(), err)
...
func (ts *UserTestSuite) TestUpdateUserEmailSuccess() {
	userA, err := NewUser("", "foo@example.com", "", "authenticated", nil, ts.OrganizationID, ts.ProjectID)
	require.NoError(ts.T(), err)
	require.NoError(ts.T(), ts.db.Create(userA, "organization_role"))
...
func (ts *UserTestSuite) TestUpdateUserEmailFailure() {
	...
	userB, err := NewUser("", "bar@example.com", "", "authenticated", nil, ts.OrganizationID, ts.ProjectID)
	require.NoError(ts.T(), err)
	require.NoError(ts.T(), ts.db.Create(userB, "organization_role"))
...
func (ts *UserTestSuite) TestNewUserWithPasswordHashSuccess() {
	...
	for _, c := range cases {
		ts.Run(c.desc, func() {
			u, err := NewUserWithPasswordHash("", "", c.hash, "", nil, ts.OrganizationID, ts.ProjectID)
			require.NoError(ts.T(), err)
...
func (ts *UserTestSuite) TestAuthenticate() {
	...
	for _, c := range cases {
		ts.Run(c.desc, func() {
			u, err := NewUserWithPasswordHash("", "", c.hash, "", nil, ts.OrganizationID, ts.ProjectID)
			require.NoError(ts.T(), err)
			require.NoError(ts.T(), ts.db.Create(u, "organization_role"))
			require.NotNil(ts.T(), u)
...
func (ts *UserTestSuite) TestCreateMultipleAdminUsersWithSameEmailDifferentProjects() {
	...
	// Create project 1
	if err := ts.setup_db.RawQuery(fmt.Sprintf("INSERT INTO auth.projects (id, name) VALUES ('%s', 'test_project_1')", projectID1)).Exec(); err != nil {
		panic(err)
	}

	// Create project 2
	if err := ts.setup_db.RawQuery(fmt.Sprintf("INSERT INTO auth.projects (id, name) VALUES ('%s', 'test_project_2')", projectID2)).Exec(); err != nil {
		panic(err)
	}
...
func (ts *UserTestSuite) TestCreateMultipleAdminUsersWithSameEmailSameProject() {
	...
	// Create project
	if err := ts.setup_db.RawQuery(fmt.Sprintf("INSERT INTO auth.projects (id, name) VALUES ('%s', 'test_project_1')", projectID)).Exec(); err != nil {
		panic(err)
	}
...
func (ts *UserTestSuite) TestCreateMultpleUsersWithSameEmailDifferentOrganizations() {
	...
	if err := ts.setup_db.RawQuery(fmt.Sprintf("INSERT INTO auth.organizations (id, name, project_id, admin_id) VALUES ('%s', 'test_organization_1', '%s', '%s')", organizationID1, projectID, adminUser1.ID)).Exec(); err != nil {
		panic(err)
	}
	...
	if err := ts.setup_db.RawQuery(fmt.Sprintf("INSERT INTO auth.organizations (id, name, project_id, admin_id) VALUES ('%s', 'test_organization_2', '%s', '%s')", organizationID2, projectID, adminUser2.ID)).Exec(); err != nil {
		panic(err)
	}

	// Create user 1
	user1, err := NewUser("", "example@example.com", "test", "", nil, organizationID1, projectID)
	require.NoError(ts.T(), err)
	require.NoError(ts.T(), ts.db.Create(user1, "organization_role"))

	// Create user 2
	user2, err := NewUser("", "example@example.com", "test", "", nil, organizationID2, projectID)
	require.NoError(ts.T(), err)
	require.NoError(ts.T(), ts.db.Create(user2, "organization_role"))
...
func (ts *UserTestSuite) TestCreateMultpleUsersWithSameEmailSameOrganization() {
	...
	if err := ts.setup_db.RawQuery(fmt.Sprintf("INSERT INTO auth.organizations (id, name, project_id, admin_id) VALUES ('%s', 'test_organization', '%s', '%s')", organizationID, projectID, adminUser.ID)).Exec(); err != nil {
		panic(err)
	}

	// Create user 1
	user1, err := NewUser("", "user@example.com", "test", "", nil, organizationID, projectID)
	require.NoError(ts.T(), err)
	require.NoError(ts.T(), ts.db.Create(user1, "organization_role"))

	// Create user 2 -> should fail
	user2, err := NewUser("", "user@example.com", "test", "", nil, organizationID, projectID)
	require.NoError(ts.T(), err)
	require.Error(ts.T(), ts.db.Create(user2, "organization_role"))
...
func (ts *UserTestSuite) TestCreateUserAndSetProjectID() {
	...
	// Create organization
	organizationID := uuid.Must(uuid.NewV4())
	if err := ts.setup_db.RawQuery(fmt.Sprintf("INSERT INTO auth.organizations (id, name, project_id, admin_id) VALUES ('%s', 'test_organization', '%s', '%s')", organizationID, projectID, user.ID)).Exec(); err != nil {
		panic(err)
	}

	// Set project ID for user, should fail
	user.ProjectID = uuid.Must(uuid.NewV4())
